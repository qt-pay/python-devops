...
class ScriptProjectViewSet(Base):
    ...
    def create(self, request, *args, **kwargs):
        try:
            ...
            abs_path = File.get_join_path(settings.TASK_SCRIPT_DIR, project_path)

            if File.if_dir_exists(abs_path):
                return json_error_response(message='项目文件夹已经存在，请检查后重测。')

            File.rm_dir(abs_path)
    ...
    def update(self, request, *args, **kwargs):
        try:
            ...
            abs_path = File.get_join_path(settings.TASK_SCRIPT_DIR, new_dir_path)
            if File.if_dir_exists(abs_path):
    ...
    def destroy(self, request, *args, **kwargs):
        try:
            ...
            if not File.get_dir_list(File.get_join_path(settings.TASK_SCRIPT_DIR, project_path)):
                File.rm_dir(File.get_join_path(settings.TASK_SCRIPT_DIR, project_path))
                instance.delete()
            else:
