...
class CloudServerView(APIView):
    def post(self, request):
        ...
        for item in result_dic:
            ...
            if not cloud_server_obj:
                ...
                if not status:
                    return new_response(code=10200, data='实例创建失败')
            else:
    ...
    def __updae_server(self, content,server_obj ):
        row_map = {'instance_id': '实例ID', 'instance_name': '实例名称', 'instance_type': '实例类型',
                   ...
                   'public_ip': '外网IP','os_version': '系统版本', 'security_groupids': '安全组', 'created_time': '创建时间', 'expired_time': '到期时间' }
        record_list = []
        ...
        if content['device_status_id'] != asset_obj.device_status_id:
            ...
            record_list.append(f' 资产状态变更:<<{baseic["instance_id"]}>> 实例状态状态,由 << {device_status_dic[old_id]} >> 变更为: << {device_status_dic[content["device_status_id"]]} >>.')
        ...
        if device_category_obj.id != asset_obj.device_type.id:
            ...
            record_list.append(f' 资产信息变更:<<{baseic["instance_id"]}>> 实例信息变更类型,由 << {old_device_type} >> 变更为: <<{device_category_obj.title}>>.')
    ...
    def __create_server(self, server_info,):
        ...
        if not device_category_obj:
            self.error_logging(content=f'实例ID:{server_info["baseic"]["instance_id"]} 类型错误: {server_info["cloud_vendors"]}.',
                               title='类型没有定义')
            return False
        ...
        if not 'zone' in server_info or not 'idc' in server_info:
            # 记录日志 数据格式错误
            self.error_logging(content=f'实例ID:{server_info["baseic"]["instance_id"]} 上传数据没有 <zone/idc> 数据.', title='数据提交格式错误')
            return False

        idc_obj = self.__check_idc_id(server_info['idc'], server_info['zone'])
        ''' 创建资产 '''
        to_day = datetime.date.today()
        asset_obj = models.Asset.objects.create(title=server_info['baseic']['instance_name'], device_status_id=server_info['device_status_id'], idc=idc_obj, device_type=device_category_obj)
        asset_obj.save()
        cloud_server_dic = {
            'asset_obj': asset_obj,
            ...
            'intranet_ip': ','.join(server_info['baseic']['intranet_ip']) if server_info['baseic']['intranet_ip'] else None,
            ...
            'security_groupids': ','.join(server_info['baseic']['security_groupids']) if server_info['baseic']['security_groupids'] else None,
        ...
        cloud_server_obj.save()
        self.access_logging(title='新增云服务器', server_obj=cloud_server_obj, content=f'实例ID:{cloud_server_obj.instance_id}, 实例名称: {cloud_server_obj.instance_name} CPU: {cloud_server_obj.cpu}')
