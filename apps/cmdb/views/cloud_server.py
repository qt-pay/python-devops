    ...
    def update(self, request, *args, **kwargs):
        ...
        try:
            row_map = {'instance_id': '实例ID', 'instance_name': '实例名称', 'instance_type': '实例类型', 'manage_ip': '管理IP',
                       'cpu': 'CPU核心数', 'memory': '制造商', 'intranet_ip': '内网IP', 'public_ip': '公网IP',
                       'os_version': '系统版本',
                       'security_groupids': '安全组', 'expired_time': '实例到期时间', 'created_time': '实例创建时间',
                       'latest_date': '更新时间', 'create_at': '创建时间', 'asset_obj': '关联资产'}
            record_list = []
            ...
            for k, v in new_data.items():
                ...
                if v != value:
                    record_list.append(
                        f' 实例ID:{instance.instance_id}: {row_map[k]},由{value if value else "Null"}变更为{v}.')
            ...
            return json_ok_response(data=instance.instance_id)
        except Exception as e:
            return json_error_response(message=str(e))
