...
class AssetViewSet(NewModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        try:
            ...
            row_map = {'title': '名称', 'business_unit': '业务线',  'device_status_id': '状态',  'cabinet_num': '机柜编号',
                       'cabinet_order': '柜上编号', 'idc': '机房', 'tag': '标签','device_type': '设备类型', 'maintain_groups': '维护组' }
            record_list =  []
            ...
            for k, v in new_data.items():
                ...
                if k == 'idc':
                    ...
                    print(v, value)
                    if v != value:
                        record_list.append(f' 名称{instance.title}: {row_map[k]},  由 {value.title if value else "Null"} 变更为 {v.title} ' )
                        #
                    ...
                    if v != value:
                        record_list.append(f' 名称{instance.title}: {row_map[k]},由 {value if value else "Null"}  变更为 {v} .')
                        setattr(instance, k, v)
    ...
    def task_asset(self, request):
        queryset = Asset.objects.filter(manage_ip__isnull=False, device_type__pid__in=[1,2], device_status_id__in=[1,2])
