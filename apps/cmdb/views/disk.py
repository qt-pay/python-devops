...
class DiskViewSet(NewModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        try:
            ...
            serializer.save()
            self.asset_record(request, '新增磁盘', f'磁盘槽位: {request.data["slot"]}, 容量: {request.data["capacity"]} 类型: {request.data["pd_type"]} ', asset_id)
    ...
    def update(self, request, *args, **kwargs):
        ...
        try:
            row_map = {'slot': '磁盘槽位', 'model': '磁盘型号',  'capacity': '磁盘容量',  'pd_type': '磁盘类型','create_at': '创建时间', 'server_obj': '关联设备'}
            record_list =  []
    ...
    def destroy(self, request, *args, **kwargs):
        try:
            ...
            instance.delete()
            self.asset_record(request, '磁盘移除', f'槽位{instance.slot}, 容量{instance.capacity}, 类型{instance.pd_type}', asset_id)
            return new_response()
        except Exception as e:
