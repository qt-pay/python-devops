...
class NetworkViewSet(Base):
    ...
    def create(self, request, *args, **kwargs):
        try:
            ...
            serializer.save()
            self.asset_record(request, '新增网卡', f'网卡名: {request.data["title"]}, 管理IP: {request.data["ipaddress"]} ',
                              asset_id)
    ...
    def update(self, request, *args, **kwargs):
        ...
        try:
            row_map = {'title': '网卡名称', 'hwaddr': 'MAC地址', 'netmask': '子网掩码', 'ipaddress': '管理IP',
                       'broadcast': '广播地址', 'up': '网卡状态', 'latest_date': '更新时间', 'create_at': '创建时间',
                       'server_obj': '关联设备'}
            record_list = []
